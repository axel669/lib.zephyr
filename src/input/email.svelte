<script>
    ...
    using an input with type="email"
    ...
    ```svelte
    <Input.Email bind:value />
    <Input.Email bind:value color="primary" />
    <Input.Email bind:value label="Wat" hint="Blep"/>
    <Input.Email bind:value bind:tvalue transform={txt => txt.toLowerCase()} />
    <Input.Email bind:value bind:valid validate={txt => txt.indexOf("a") > 0} />
    ```
    */
