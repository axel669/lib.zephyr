<script>
    ...
    using an input with type="search"
    ...
    ```svelte
    <Input.Search bind:value />
    <Input.Search bind:value color="primary" />
    <Input.Search bind:value label="Wat" hint="Blep"/>
    <Input.Search bind:value bind:tvalue transform={txt => txt.toLowerCase()} />
    <Input.Search bind:value bind:valid validate={txt => txt.indexOf("a") > 0} />
    ```
    */
